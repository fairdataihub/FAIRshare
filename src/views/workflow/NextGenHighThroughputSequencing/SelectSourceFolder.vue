  ...
  <div class="flex h-full w-full max-w-screen-xl flex-col items-center justify-center p-3 pr-5">
    <div class="flex h-full w-full flex-col">
      ...
      <div class="item-center flex justify-center gap-8 pt-8">
        ...
        <!-- hidden for now. Will probably be replaced by another GEO entry -->
        <div class="hidden">
          <div
            class="single-check-box flex h-[200px] w-[200px] cursor-pointer flex-col items-center justify-evenly rounded-lg p-4 shadow-md transition-all"
...
export default {
  name: "NextGenHighThroughputSequencingSelectSourceFolder",
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    startCuration() {
      ...
      if (this.locationID === "github") {
        this.$router.push(
          `/datasets/${this.datasetID}/${this.workflowID}/NextGenHighThroughputSequencing/selectGithubRepo`
        );
      } else {
        this.$router.push({
          path: `/datasets/${this.dataset.id}/${this.workflowID}/NextGenHighThroughputSequencing/reviewStandards`,
        });
      }
    },
  },
