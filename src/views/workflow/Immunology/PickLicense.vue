  ...
  <div class="flex h-full w-full max-w-screen-xl flex-col items-center justify-center p-3 pr-5">
    <div class="flex h-full w-full flex-col">
      <span class="text-left text-lg font-medium">
        Select a license that defines the terms under which your files can be used
      </span>
      ...
      <div class="flex w-full flex-row justify-center space-x-4 py-2">
        <router-link
          :to="`/datasets/${this.$route.params.datasetID}/${this.$route.params.workflowID}/Immunology/createMetadata`"
          class=""
        >
  ...
  methods: {
    ...
    startCuration() {
      this.$refs.licenseForm.validate((valid) => {
        if (valid) {
          this.dataset.data.Immunology.questions.license = this.licenseForm.license;
          this.dataset.data.general.questions.license = this.licenseForm.license;
