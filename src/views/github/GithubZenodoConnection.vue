  ...
  export default {
    ...
    methods: {
      ...
      async createZenodoJsonFile() {
          ...
          if ("dates" in zenodoMetadata.conference) {
            if (zenodoMetadata.conference.dates.length === 2) {
              metadata.conference_dates =
                dayjs(zenodoMetadata.conference.dates[0]).format(
                  "MMMM D, YYYY"
                ) +
                " - " +
                dayjs(zenodoMetadata.conference.dates[1]).format(
                  "MMMM D, YYYY"
                );
            }
          }
        ...
        if (
          "subjects" in zenodoMetadata &&
          zenodoMetadata.subjects.length > 0
        ) {
          metadata.subjects = [];
    ...
    async mounted() {
      ...
      this.zenodoData = this.jsonToTableDataRecursive(
        this.zenodoData,
        1,
        "ROOT"
      );
